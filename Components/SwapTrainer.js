...
const SwapTrainer = ({ showModal, setShowModal, itemId, getAllSchedule,trainerName }) => {
  ...
  // console.log("first  allSchedule---> in modal-->",allSchedule)
  return (
    <div>
          <ToastContainer />
        ...
        <Modal.Body className='trainnext-body'>
          ...
          >
            ...
            {allTrainer?.map((item,idx)=>{
              return(
                <option value={item?.id} selected={item?.trainerName == trainerNames ? true : false}>{item.trainerName}</option>
              )
            })}
